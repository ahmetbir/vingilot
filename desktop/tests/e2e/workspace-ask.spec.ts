...
async function stubBackend(page: Page, agent: "ready" | "not-configured") {
  await page.evaluate(
    ({ agent, answer }) => {
      const internals = (
...
async function openWorktree(page: Page, agent: "ready" | "not-configured") {
