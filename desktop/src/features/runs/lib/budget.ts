// Pure Run-detail logic: legal next transitions and the budget-honesty
// render-model. No React, no coordinator-client imports — this is the thing
// RunDetail.tsx and BudgetBar.tsx both consume.
//
// Ported verbatim from vingilot/workbench/src/run/budget.ts (ADR-001's
// 2026-08-03 reversal) — the sibling app is donor code once this lands.

import { wallClock } from "./runModel.ts";
import type { RunStatus, RunSummary } from "./runModel.ts";
    ...
    wc === null
      ...
      : {
          pct:
            wc.limitSecs === null
              ? 0
              : Math.min(1, wc.spentSecs / wc.limitSecs),
          label:
            wc.limitSecs === null
              ? `${wc.spentSecs}s`
...
function lagLabel(observedAt: string, now: Date): string {
  const lagSecs = Math.max(
    0,
    Math.floor((now.getTime() - new Date(observedAt).getTime()) / 1000),
  );
  return `${lagSecs}s`;
}
