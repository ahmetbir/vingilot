...
function parseNotificationTarget(
  ...
  return {
    ...
    pubkey,
    threadRootId,
    // Only when there really is one. The field is this fork's, and a message
    // target has no worktree — spelling it `worktreeId: null` on every parse
    // changed the shape upstream's own test deep-equals against, which is a
    // divergence bought for nothing. Optional in the type, so absent is a
    // legal answer.
