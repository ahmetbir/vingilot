...
mod commands;
// `pub(crate)` rather than private: dictation builds an `SttPipeline` too, and
// after the upstream sync that constructor takes a `HumanFloor`. Dictation's is
// its own and shared with nobody — there is no second speaker in a dictation
// session — but it still has to be able to name the type.
pub(crate) mod human_floor;
pub mod jitter;
#[cfg(test)]
mod latency_bench;
mod local_barge_in;
