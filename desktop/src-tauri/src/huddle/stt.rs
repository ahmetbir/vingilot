...
impl SttPipeline {
    ...
    ///
    /// Reconciled with the upstream sync by hand: that round replaced
    /// `tts_active` with `human_floor` + `output_device` and changed what the
    /// worker receives, while this fork added the language. Both changes are
    /// real, so this signature carries upstream's parameters and P8's.
...
fn stt_worker(
    model_dir: PathBuf,
    // P8's language selector, kept over upstream's newer audio input type: the
    // sync changed what the worker receives (`Vec<u8>` → `SttAudioInput`) while
    // this fork added which language it should transcribe. Neither displaces
    // the other.
    language: SttLanguage,
    audio_rx: Receiver<SttAudioInput>,
