...
pub(crate) fn settings_path(app: &AppHandle) -> Result<PathBuf, String> {
    app.path()
        ...
        .map_err(|error| format!("could not locate Vingilot settings storage: {error}"))
}
...
pub(crate) fn load_from_path(path: &Path) -> Result<TtsSettings, String> {
    ...
    if version > u64::from(CURRENT_VERSION) {
        return Err(format!(
            "text-to-speech settings version {version} is newer than this Vingilot build supports"
        ));
    }
...
mod tests {
    ...
    fn rejects_future_schema_versions_clearly() {
        ...
        assert!(load_from_path(&path)
            ...
            .contains("newer than this Vingilot build supports"));
    }
