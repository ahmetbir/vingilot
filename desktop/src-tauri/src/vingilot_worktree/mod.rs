...
mod tests {
    ...
    /// A real repository, because the thing under test is git's behaviour at
    /// this app's boundary — a mocked git would prove only that this module
    /// can call a mock. Global and system config are switched off so the
    /// owner's own git settings cannot change what these tests observe.
    ///
    /// Teardown is `TempDir`'s `Drop` and, where a test needs it, `git
    /// worktree remove` itself. Nothing here removes a directory by hand.
    struct Repo {
        dir: TempDir,
    }
    ...
    /// Somewhere to hang worktrees off, outside the repository — where the
    /// real worktree root is, and the layout `git worktree add` is happiest
    /// with.
    fn worktree_path(root: &TempDir, name: &str) -> String {
        root.path()
            .join("trees")
            .join(name)
            .to_string_lossy()
