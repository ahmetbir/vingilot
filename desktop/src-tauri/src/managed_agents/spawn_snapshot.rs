...
use super::{
    ...
    effective_mcp_command, known_acp_runtime, normalize_agent_args,
...
impl SpawnConfigSnapshot {
    ...
    pub(crate) fn from_inputs(inputs: SpawnConfigInputs<'_>) -> Self {
        ...
        Self {
            ...
            // Vingilot (seams.yaml: spawn_hash -> spawn_snapshot port): route the
            // spawn snapshot's mcp_command through effective_mcp_command so an
            // unclassified preset/custom harness resolves to buzz-dev-mcp rather
