...
pub(super) fn run(
    ...
    // Observe PATH changes and binaries added after Vingilot launched.
...
fn failure(
    ...
    Some(InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "The installer finished, but Vingilot still could not use {runtime_id} (observed: {observed})."
        ),
        exit_code: None,
        hint: Some(
            "Vingilot requires the vendor CLI executable, not only its desktop app. If the CLI was installed while Vingilot was open, restart Vingilot and check again."
                .to_string(),
        ),
    })
}
