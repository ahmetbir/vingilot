...
async fn tokio_main() -> Result<()> {
    ...
    // Upstream's inbound author gate, restored by hand after the sync. Both
    // sides added code at this exact point — upstream this declaration, the
    // fork the broker below — so `-X ours` kept the broker and dropped these
    // two lines, leaving the call site further down referring to a binding
    // that no longer existed. Nothing about the two additions conflicts; they
    // were only neighbours.
