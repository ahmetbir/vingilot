...
import { railGroups, statusClass, wallClock } from "./run.ts";
...
test("statusClass is total over all 10 statuses", () => {
  for (const status of ALL_STATUSES) {
    ...
    assert.ok(["live", "ok", "attn", "stop", "muted"].includes(cls), `status ${status} -> ${cls}`);
  }
});

test("wallClock is null when never started", () => {
  const r = run({ status: "ready", wall_limit_secs: 60, wall_started_at: null });
...
test("wallClock reports exceeded exactly at the boundary", () => {
  ...
  const noLimit = wallClock(run({ status: "running", wall_started_at: "2026-01-01T00:00:00Z" }), new Date("2026-01-01T00:05:00Z"));
