...
row. The team thread's composer (`lib/composerKeys.ts`): `⌘↵` — and `⌃↵` —
sends, and a bare `↵` is a newline, because a message here carries a path and
goes to a server. **Both were `onKeyDown` handlers inside their components
until the cheatsheet went in**, which is exactly why they are modules now: the
sheet is generated by asking the `resolve*` maps what they answer to, so a
chord bound in a component is a chord it cannot print — and it opens by
claiming it carries every chord this workspace binds. The composer is the
stop-gap upstream's own composer replaces, and its map is expected to go the
same way its chord does.
